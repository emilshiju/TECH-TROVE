...
export default {
  getHomePage: asyncHandler(async (req, res, next) => {
    try {
      ...
      console.log(
        "wishhhhhhhhhhhhhhlisssssssssssssssssssstttttttttttttttttttttttttttttttttttt"
      );
      ...
      const Products = await Product.find({ status: { $ne: true } }).populate(
        "category"
      );
  ...
  registerUser: asyncHandler(async (req, res, next) => {
    try {
      ...
      if (mobile) {
        return res.render("user/user-Register", {
          message: "phone number already existst",
        });
      }
      if (req.body.password != req.body.confpassword) {
        return res.render("user/user-Register", {
          message: "password and confmpassword  check",
        });
      }
    ...
    try {
      ...
      if (user && user.is_blocked) {
        ...
        return res.render("user/user-Login", {
          message: "your permssion declined",
        });
      }
      ...
      if (user) {
        if (password.length == 4) {
          ...
          const verified = await verificationHelper.verifyOtp(
            user.mobile,
            password
          );
          if (verified) {
            ...
            console.log(
              "addddddddddddddddddddddddddddddddddddddddddddd",
              req.session.user
            );
  ...
  otpLogin: async (req, res) => {
    try {
      ...
      console.log(
        "poyiiiiiiiiippppppppooooooooooooooyyyiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii"
      );
      console.log(phone);
      await verificationHelper.sendOtp(phone);
    } catch (error) {
  ...
  verifyOTP: async (req, res) => {
    try {
      ...
      bcrypt.hash(user.password, saltRounds, (err, hash) => {
        if (err) {
          res.status(500).send({
            message: err.message || "errors occured while hashing",
          });
          return;
        }
  ...
  displayProduct: async (req, res, next) => {
    try {
      ...
      const minPriceString = req.query.minPrice
        ? req.query.minPrice.replace(/,/g, "")
        : "";
      const maxPriceString = req.query.maxPrice
        ? req.query.maxPrice.replace(/,/g, "")
        : "";
  ...
  changePasswordOtp: asyncHandler(async (req, res, next) => {
    try {
      ...
      const user = await User.findOne({ email: email });
      if (!user)
        return res.json({ message: "enter valid email ", status: false });
      const phone = user.mobile;
